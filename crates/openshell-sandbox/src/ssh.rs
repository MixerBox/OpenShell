...
impl russh::server::Handler for SshHandler {
    ...
    async fn channel_open_direct_tcpip(
        ...
        tokio::spawn(async move {
            ...
            let tcp = match crate::netns_connect::connect_in_netns(&addr, netns_fd).await {
                Ok(stream) => stream,
                Err(err) => {
...
// `connect_in_netns` has been extracted to crate::netns_connect for reuse
// by both SSH direct-tcpip forwarding and TCP port forwarding.
